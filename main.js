...
/* =====================================================
   ...
   Document Studio Tools Dashboard + Universal Printable Engine
   - 6 document cards
   - Loading transition per document
   - Image/PDF workspace, page select, zoom, 8-handle crop
   - Front + Back A4 layout, 2.2mm top center gap
   - Passport studio uses same professional crop pattern
===================================================== */
(function(){
  const VERSION='v41.9-PDF-Selection-Fix';
  const CARD_W=85.6, CARD_H=54, TOP=2.2, GAP=2.4, RATIO=CARD_W/CARD_H;
  const PASS_W=35, PASS_H=45, PASS_RATIO=PASS_W/PASS_H;
  const DOCS={
    aadhaar:{name:'Aadhaar Card',icon:'aadhaar.jpg',hint:'PDF/Image se printable Aadhaar'},
    pan:{name:'PAN Card',icon:'pan.jpg',hint:'PAN front/back printable'},
    voter:{name:'Voter ID',icon:'voter.jpg',hint:'Voter card print layout'},
    dl:{name:'Driving Licence',icon:'dl.jpg',hint:'DL card print layout'},
  ...
  const S={doc:null,mode:'both',tab:'pdf',pdfDoc:null,pdfPages:0,page:1,scale:1.35,front:null,back:null,pdfCrop:null,frontCrop:null,backCrop:null,docPdf:null,passImg:null,passCrop:null,passPdf:null};
  const $=s=>document.querySelector(s); const $$=s=>Array.from(document.querySelectorAll(s));
  function ws(){return document.getElementById('workspace')}
  function toastX(m){const t=document.getElementById('toast'); if(t){t.textContent=m;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2500)}else alert(m)}
  function header(t,sub){return typeof pageHeader==='function'?pageHeader(t,sub):`<div class="page-head"><div><h1>${t}</h1><p>${sub||''}</p></div><span class="pro-badge">${VERSION}</span></div>`}
  ...
  function canAccess(tool){return typeof v40EnsureAuth==='function'?v40EnsureAuth(tool):true}

  window.documentStudio=function(){
    if(!canAccess('documentStudio')) return;
    S.doc=null;
    const cards=Object.entries(DOCS).map(([id,d])=>`<button class="v418-doc-card" onclick="v418OpenDoc('${id}')"><img src="${d.icon}" onerror="this.style.display='none'"><h3>${d.name}</h3><p>${d.hint}</p><small>Open Editor →</small></button>`).join('');
    ws().innerHTML=header('Document Studio','6 document tools: Aadhaar, PAN, Voter, DL, ABHA and Ayushman.')+`
      <div class="card v418-hero"><div><h2>Choose Document</h2><p>Document pe click karte hi uska full dashboard open hoga: PDF upload, page select, zoom, professional crop, preview, download and print.</p></div><button onclick="v418Roadmap()">Roadmap</button></div>
      <div class="v418-doc-grid">${cards}</div>`;
  };
  window.v418Roadmap=function(){toastX('Phase 1: Document Studio universal printable engine active. Phase 2: CamScanner auto crop/edge detection next.');}
  window.v418OpenDoc=function(id){S.doc=id; S.docPdf=null; ws().innerHTML=header('Opening '+DOCS[id].name,'Loading editor workspace...')+`<div class="card v418-loading"><span></span><b>Preparing ${DOCS[id].name} tools</b><p>PDF preview, crop handles and print engine loading...</p></div>`; setTimeout(renderEditor,350)};
  function modeBtns(){return [['front','Front'],['back','Back'],['both','Front + Back']].map(([k,v])=>`<button class="${S.mode===k?'active':''}" onclick="v418Mode('${k}')">${v}</button>`).join('')}
  function tabBtns(){return [['pdf','PDF Upload'],['image','Image Upload']].map(([k,v])=>`<button class="${S.tab===k?'active':''}" onclick="v418Tab('${k}')">${v}</button>`).join('')}
  window.v418Mode=m=>{S.mode=m;renderEditor()}; window.v418Tab=t=>{S.tab=t;renderEditor()};
  function renderEditor(){
    const d=DOCS[S.doc||'aadhaar'];
    const effectiveMode = S.tab==='pdf' ? 'single' : S.mode;
    const front=effectiveMode!=='back', back=effectiveMode!=='front';
    ws().innerHTML=header(d.name+' Printable Editor','Large workspace: upload, zoom, 8-point crop, preview, download and print.')+`
      <div class="card v418-head"><div class="v418-title"><img src="${d.icon}" onerror="this.style.display='none'"><div><h2>${d.name}</h2><p>Card size 85.6 × 54 mm · top-center margin 2.2 mm · lamination/fold ready</p></div></div><div class="v418-actions"><button onclick="documentStudio()">← All Documents</button><button class="blue" onclick="v418Preview()">Preview</button><button class="green" onclick="v418Download()">Download PDF</button><button onclick="v418Print()">Print</button></div></div>
      <div class="v418-work">
        <section>
          <div class="card"><h3>1. Mode + Upload</h3><div class="v418-tabs">${modeBtns()}</div><div class="v418-tabs">${tabBtns()}</div>${S.tab==='pdf'?pdfUploader():imageUploader(front,back)}</div>
          <div class="card"><h3>2. Professional Crop Tool</h3>${S.tab==='pdf'?pdfWorkspace():imageWorkspace(front,back)}<div class="v418-note">Tip: corner dots se resize, side ke “|” handle se width/height adjust, center se move. Rotate buttons tirchi/galat orientation ko fix karte hain.</div></div>
        </section>
        <aside class="v418-side"><div class="card"><h3>Live A4 Print Preview</h3><div class="v418-a4"><div id="v418Strip" class="v418-strip"></div></div><div class="v418-spec"><div><span>Paper</span><b>A4 Portrait</b></div><div><span>Top Gap</span><b>2.2 mm</b></div><div><span>Alignment</span><b>Top Center</b></div><div><span>PDF Mode</span><b>Single printable card</b></div></div><label class="v418-copy">Copies <select id="v418Copies" onchange="v418Preview()"><option value="1">1</option><option value="2">2</option><option value="4">4</option><option value="6">6</option></select></label><div class="v418-actions bottom"><button class="blue" onclick="v418Preview()">Preview</button><button class="green" onclick="v418Download()">Download</button><button onclick="v418Print()">Print</button></div><p class="v418-warn">Print me scale <b>Actual Size / 100%</b> rakhein.</p></div></aside>
      </div>`;
    setTimeout(initCrops,120); setTimeout(()=>{if(S.tab==='pdf'&&S.pdfDoc)renderPdf(); v418Preview();},260);
  }
  function pdfUploader(){return `<label class="v418-drop"><input hidden type="file" accept="application/pdf" onchange="v418LoadPdf(event)">${S.pdfDoc?`PDF loaded · ${S.pdfPages} page(s)`:'Upload official PDF / full page PDF'}</label>${S.pdfDoc?`<div class="v418-tools"><label>Page <select onchange="v418Page(this.value)">${Array.from({length:S.pdfPages},(_,i)=>`<option value="${i+1}" ${S.page===i+1?'selected':''}>${i+1}</option>`).join('')}</select></label><button onclick="v418Zoom(1.2)">Zoom +</button><button onclick="v418Zoom(.84)">Zoom -</button><button onclick="v418Fit()">Fit</button></div>`:''}`}
  function imageUploader(front,back){return `<div class="v418-upload-grid">${front?`<label class="v418-drop"><input hidden type="file" accept="image/*" onchange="v418LoadImage(event,'front')">${S.front?`<img src="${S.front}"><b>Front loaded</b>`:'Upload Front Image'}</label>`:''}${back?`<label class="v418-drop"><input hidden type="file" accept="image/*" onchange="v418LoadImage(event,'back')">${S.back?`<img src="${S.back}"><b>Back loaded</b>`:'Upload Back Image'}</label>`:''}</div>`}
  function pdfWorkspace(){return `<div id="v418PdfStage" class="v418-stage large v419-pdf-stage ${S.pdfDoc?'':'empty'}">${S.pdfDoc?'<canvas id="v418PdfCanvas"></canvas>':'Upload PDF first. PDF ka selected page yahi bada preview hoga.'}</div><div class="v418-tools"><button onclick="v418Auto('pdf')">Auto Crop</button><button onclick="v418Zoom(1.2)">Zoom +</button><button onclick="v418Zoom(.84)">Zoom -</button><button onclick="v418Fit()">Fit Width</button><button onclick="v418Reset('pdf')">Reset Crop</button></div><div class="v418-note">PDF upload mode me preview single printable card hota hai. Front + Back layout sirf Image Upload me front/back image add karne ke baad banega.</div>`}
  function imageWorkspace(front,back){return `<div class="v418-upload-grid">${front?panel('front','Front'):' '}${back?panel('back','Back'):' '}</div>`}
  function panel(side,label){const img=S[side]; return `<div><b>${label} Crop</b><div id="v418${cap(side)}Stage" class="v418-stage ${img?'':'empty'}">${img?`<img id="v418${cap(side)}Img" src="${img}">`:`Upload ${label} image first`}</div><div class="v418-tools"><button onclick="v418Auto('${side}')">Auto Crop</button><button onclick="v418Rotate('${side}',-90)">Left</button><button onclick="v418Rotate('${side}',90)">Right</button><button onclick="v418Reset('${side}')">Reset</button></div></div>`}
  function cap(s){return s.charAt(0).toUpperCase()+s.slice(1)}

  async function normalize(file){const src=await readFile(file); const img=await loadImg(src); const max=2400, sc=Math.min(1,max/Math.max(img.width,img.height)); const c=document.createElement('canvas'),ctx=c.getContext('2d'); c.width=Math.round(img.width*sc); c.height=Math.round(img.height*sc); ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height); return c.toDataURL('image/jpeg',.95)}
  window.v418LoadImage=async(e,side)=>{const f=e.target.files[0]; if(!f)return; S[side]=await normalize(f); S[side+'Crop']=null; renderEditor(); setTimeout(()=>v418Auto(side),320)};
  window.v418LoadPdf=async e=>{const f=e.target.files[0]; if(!f)return; if(!window.pdfjsLib){toastX('PDF library load nahi hui. Internet/refresh check karo.');return} S.pdfDoc=await pdfjsLib.getDocument({data:await f.arrayBuffer()}).promise; S.pdfPages=S.pdfDoc.numPages; S.page=1; S.pdfCrop=null; renderEditor(); setTimeout(renderPdf,250)};
  window.v418Page=v=>{S.page=Number(v);S.pdfCrop=null;renderEditor();setTimeout(renderPdf,220)}; window.v418Zoom=f=>{S.scale=Math.max(.55,Math.min(4,S.scale*f));renderPdf()}; window.v418Fit=()=>{S.scale=1.35;renderPdf()};
  async function renderPdf(){const c=$('#v418PdfCanvas'); if(!c||!S.pdfDoc)return; const p=await S.pdfDoc.getPage(S.page); const vp=p.getViewport({scale:S.scale}); c.width=vp.width;c.height=vp.height; await p.render({canvasContext:c.getContext('2d'),viewport:vp}).promise; await raf2(); S.pdfCrop=createCrop($('#v418PdfStage'),c,RATIO,S.pdfCrop);}
  function initCrops(){ if(S.tab==='image'){ if(S.front&&$('#v418FrontImg'))S.frontCrop=createCrop($('#v418FrontStage'),$('#v418FrontImg'),RATIO,S.frontCrop); if(S.back&&$('#v418BackImg'))S.backCrop=createCrop($('#v418BackStage'),$('#v418BackImg'),RATIO,S.backCrop); } }
  function mediaBox(stage,media){const sr=stage.getBoundingClientRect(), mr=media.getBoundingClientRect(); return {x:mr.left-sr.left,y:mr.top-sr.top,w:mr.width,h:mr.height}}
  function clamp(st,x=st.x,y=st.y,w=st.w,h=st.h,ratio=st.ratio){const b=mediaBox(st.stage,st.media); w=Math.max(70,Math.min(w,b.w)); h=Math.max(44,Math.min(h,b.h)); if(w/h>ratio)h=w/ratio; else w=h*ratio; if(w>b.w){w=b.w;h=w/ratio} if(h>b.h){h=b.h;w=h*ratio} x=Math.max(b.x,Math.min(x,b.x+b.w-w)); y=Math.max(b.y,Math.min(y,b.y+b.h-h)); return {x,y,w,h}}
  function createCrop(stage,media,ratio,old){ if(!stage||!media)return null; stage.querySelectorAll('.v418-crop').forEach(e=>e.remove()); const b=mediaBox(stage,media); let base=old||{}; let w=base.w||b.w*.86,h=base.h||w/ratio; if(h>b.h*.82){h=b.h*.82;w=h*ratio} const st={stage,media,ratio,x:base.x??(b.x+(b.w-w)/2),y:base.y??(b.y+(b.h-h)/2),w,h}; const box=document.createElement('div'); box.className='v418-crop'; ['nw','n','ne','e','se','s','sw','w'].forEach(h=>{const sp=document.createElement('span');sp.className='v418-h '+h;sp.dataset.h=h;box.appendChild(sp)}); const mv=document.createElement('i');mv.className='v418-move';mv.textContent='MOVE';box.appendChild(mv); st.box=box;stage.appendChild(box); bindCrop(st); paint(st); return st;}
  function paint(st){Object.assign(st,clamp(st)); Object.assign(st.box.style,{left:st.x+'px',top:st.y+'px',width:st.w+'px',height:st.h+'px'});}
  function bindCrop(st){let mode='',start={}; const pos=e=>e.touches?{x:e.touches[0].clientX,y:e.touches[0].clientY}:{x:e.clientX,y:e.clientY}; const down=e=>{e.preventDefault();e.stopPropagation(); mode=e.target.dataset.h||'move'; const p=pos(e); start={px:p.x,py:p.y,x:st.x,y:st.y,w:st.w,h:st.h}; document.addEventListener('mousemove',move);document.addEventListener('mouseup',up);document.addEventListener('touchmove',move,{passive:false});document.addEventListener('touchend',up)}; const move=e=>{e.preventDefault(); const p=pos(e),dx=p.x-start.px,dy=p.y-start.py; let x=start.x,y=start.y,w=start.w,h=start.h; if(mode==='move'){x+=dx;y+=dy}else{if(mode.includes('e'))w+=dx;if(mode.includes('s'))h+=dy;if(mode.includes('w')){x+=dx;w-=dx}if(mode.includes('n')){y+=dy;h-=dy}} Object.assign(st,clamp(st,x,y,w,h)); paint(st);}; const up=()=>{document.removeEventListener('mousemove',move);document.removeEventListener('mouseup',up);document.removeEventListener('touchmove',move);document.removeEventListener('touchend',up); v418Preview();}; st.box.addEventListener('mousedown',down); st.box.addEventListener('touchstart',down,{passive:false});}
  window.v418Auto=side=>{const st=S[side+'Crop']; if(!st){initCrops(); if(side==='pdf')renderPdf(); return} const b=mediaBox(st.stage,st.media); Object.assign(st,{x:b.x+b.w*.06,y:b.y+b.h*.08,w:b.w*.88,h:(b.w*.88)/st.ratio}); paint(st); v418Preview(); toastX('Auto crop applied. Ab handles se fine tune karo.');}
  window.v418Reset=side=>{S[side+'Crop']=null; if(side==='pdf')renderPdf(); else renderEditor();}
  window.v418Rotate=async(side,deg)=>{if(!S[side])return; const img=await loadImg(S[side]); const c=document.createElement('canvas'),ctx=c.getContext('2d'); c.width=Math.abs(deg)%180===90?img.height:img.width; c.height=Math.abs(deg)%180===90?img.width:img.height; ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height); ctx.translate(c.width/2,c.height/2); ctx.rotate(deg*Math.PI/180); ctx.drawImage(img,-img.width/2,-img.height/2); S[side]=c.toDataURL('image/jpeg',.95); S[side+'Crop']=null; renderEditor();}
  async function crop(st){ if(!st)return null; const m=st.media, mr=m.getBoundingClientRect(), sr=st.stage.getBoundingClientRect(); let rx=(st.x-(mr.left-sr.left))/mr.width, ry=(st.y-(mr.top-sr.top))/mr.height, rw=st.w/mr.width, rh=st.h/mr.height; rx=Math.max(0,Math.min(1,rx)); ry=Math.max(0,Math.min(1,ry)); rw=Math.max(.01,Math.min(rw,1-rx)); rh=Math.max(.01,Math.min(rh,1-ry)); let source=m, sw=m.width, sh=m.height; if(m.tagName==='IMG'){source=await loadImg(m.src); sw=source.width; sh=source.height} const c=document.createElement('canvas'),ctx=c.getContext('2d'); c.width=1600; c.height=Math.round(c.width/RATIO); ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height); ctx.drawImage(source,sw*rx,sh*ry,sw*rw,sh*rh,0,0,c.width,c.height); return c.toDataURL('image/jpeg',.96)}
  async function collect(){const arr=[]; if(S.tab==='pdf'){const src=await crop(S.pdfCrop); if(src)arr.push({src,label:'PRINTABLE'}); } else {if(S.mode!=='back')arr.push({src:await crop(S.frontCrop),label:'FRONT'}); if(S.mode!=='front')arr.push({src:await crop(S.backCrop),label:'BACK'});} return arr.filter(x=>x.src);}
  window.v418SetPdfSide=async side=>{const src=await crop(S.pdfCrop); if(!src)return toastX('Pehle PDF crop select karo'); S[side]=src; S.tab='image'; toastX((side==='front'?'Front':'Back')+' set ho gaya. Back/Front bhi set kar sakte ho.'); renderEditor();}
  window.v418Preview=async()=>{const box=$('#v418Strip'); if(!box)return; const imgs=await collect(); box.innerHTML=imgs.length?imgs.map(x=>`<div><img src="${x.src}"><small>${x.label}</small></div>`).join(''):`<em>Upload aur crop karne ke baad preview yahan dikhega</em>`;};
  async function makeDocPdf(){const imgs=await collect(); if(!imgs.length){toastX('Upload aur crop document first');return null} const {jsPDF}=window.jspdf; const pdf=new jsPDF({unit:'mm',format:'a4'}); const copies=Number($('#v418Copies')?.value||1); let y=TOP; for(let c=0;c<copies;c++){const total=imgs.length*CARD_W+(imgs.length-1)*GAP; let x=(210-total)/2; imgs.forEach((im,i)=>{pdf.addImage(im.src,'JPEG',x,y,CARD_W,CARD_H); pdf.setDrawColor(0); pdf.setLineWidth(.25); pdf.rect(x,y,CARD_W,CARD_H); pdf.setFontSize(6); pdf.text(im.label,x+CARD_W/2,y+CARD_H+2.5,{align:'center'}); x+=CARD_W+GAP;}); y+=CARD_H+GAP+5; if(y+CARD_H>292&&c<copies-1){pdf.addPage();y=TOP;}} pdf.setFontSize(7); pdf.setTextColor(90); pdf.text('Smart Photo Toolkit Pro '+VERSION+' · Print Actual Size / 100%',105,292,{align:'center'}); S.docPdf=pdf; return pdf;}
  window.v418Download=async()=>{const pdf=await makeDocPdf(); if(pdf)pdf.save((S.doc||'document')+'-printable-a4.pdf')};
  window.v418Print=async()=>{const pdf=await makeDocPdf(); if(!pdf)return; pdf.autoPrint(); window.open(URL.createObjectURL(pdf.output('blob')),'_blank')};

  window.passportStudio=function(){
    ws().innerHTML=header('Passport Photo Studio','Same professional crop engine: upload, adjust, preview, download and print 35×45 mm photos.')+`
    <div class="v418-work"><section><div class="card"><h3>1. Upload Photo</h3><label class="v418-drop"><input hidden type="file" accept="image/*" onchange="v418LoadPass(event)">${S.passImg?`<img src="${S.passImg}"><b>Photo loaded</b>`:'Upload Passport Photo'}</label></div><div class="card"><h3>2. Select Printable Area</h3><div id="v418PassStage" class="v418-stage large ${S.passImg?'':'empty'}">${S.passImg?`<img id="v418PassImg" src="${S.passImg}">`:'Upload photo first'}</div><div class="v418-tools"><button onclick="v418PassAuto()">Auto Crop</button><button onclick="v418PassRotate(-90)">Left</button><button onclick="v418PassRotate(90)">Right</button><button onclick="v418PassReset()">Reset</button></div></div></section><aside class="v418-side"><div class="card"><h3>A4 Passport Preview</h3><div class="v418-a4"><div id="v418PassStrip" class="v418-pass-strip"></div></div><label class="v418-copy">Photos <select id="v418PassCount" onchange="v418PassPreview()"><option value="5">5</option><option value="6">6</option><option value="8">8</option><option value="12">12</option></select></label><div class="v418-actions bottom"><button class="blue" onclick="v418PassPreview()">Preview</button><button class="green" onclick="v418PassDownload()">Download</button><button onclick="v418PassPrint()">Print</button></div><p class="v418-warn">Passport print bhi top-center 2.2mm se start hota hai.</p></div></aside></div>`;
    setTimeout(()=>{if(S.passImg&&$('#v418PassImg')){S.passCrop=createCrop($('#v418PassStage'),$('#v418PassImg'),PASS_RATIO,S.passCrop); v418PassPreview();}},130);
  };
  window.v418LoadPass=async e=>{const f=e.target.files[0]; if(!f)return; S.passImg=await normalize(f);S.passCrop=null;passportStudio();setTimeout(v418PassAuto,300)};
  window.v418PassAuto=()=>{if(!S.passCrop&&$('#v418PassImg'))S.passCrop=createCrop($('#v418PassStage'),$('#v418PassImg'),PASS_RATIO,null); const st=S.passCrop;if(!st)return; const b=mediaBox(st.stage,st.media); let h=b.h*.84,w=h*PASS_RATIO; if(w>b.w*.72){w=b.w*.72;h=w/PASS_RATIO} Object.assign(st,{x:b.x+(b.w-w)/2,y:b.y+(b.h-h)/2,w,h}); paint(st); v418PassPreview();}
  window.v418PassReset=()=>{S.passCrop=null;passportStudio()};
  window.v418PassRotate=async deg=>{if(!S.passImg)return; const img=await loadImg(S.passImg); const c=document.createElement('canvas'),ctx=c.getContext('2d'); c.width=Math.abs(deg)%180===90?img.height:img.width;c.height=Math.abs(deg)%180===90?img.width:img.height;ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.translate(c.width/2,c.height/2);ctx.rotate(deg*Math.PI/180);ctx.drawImage(img,-img.width/2,-img.height/2);S.passImg=c.toDataURL('image/jpeg',.95);S.passCrop=null;passportStudio()};
  async function cropPass(){const st=S.passCrop;if(!st)return null; const m=st.media,mr=m.getBoundingClientRect(),sr=st.stage.getBoundingClientRect(); let rx=(st.x-(mr.left-sr.left))/mr.width,ry=(st.y-(mr.top-sr.top))/mr.height,rw=st.w/mr.width,rh=st.h/mr.height; const img=await loadImg(m.src); const c=document.createElement('canvas'),ctx=c.getContext('2d'); c.width=700;c.height=900;ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,img.width*rx,img.height*ry,img.width*rw,img.height*rh,0,0,c.width,c.height); return c.toDataURL('image/jpeg',.96)}
  window.v418PassPreview=async()=>{const box=$('#v418PassStrip'); if(!box||!S.passCrop)return; const src=await cropPass(); const n=Number($('#v418PassCount')?.value||5); box.innerHTML=Array.from({length:n},()=>`<img src="${src}">`).join('')};
  async function makePassPdf(){if(!S.passCrop){toastX('Upload aur crop photo first');return null} const src=await cropPass(); const n=Number($('#v418PassCount')?.value||5); const {jsPDF}=window.jspdf; const pdf=new jsPDF({unit:'mm',format:'a4'}); let x=5,y=TOP,g=3; for(let i=0;i<n;i++){if(x+PASS_W>205){x=5;y+=PASS_H+g} pdf.addImage(src,'JPEG',x,y,PASS_W,PASS_H);pdf.rect(x,y,PASS_W,PASS_H);x+=PASS_W+g;} S.passPdf=pdf; return pdf;}
  window.v418PassDownload=async()=>{const pdf=await makePassPdf(); if(pdf)pdf.save('passport-photo-35x45-a4.pdf')}; window.v418PassPrint=async()=>{const pdf=await makePassPdf(); if(!pdf)return; pdf.autoPrint(); window.open(URL.createObjectURL(pdf.output('blob')),'_blank')};

  const prevShow=window.showTool;
