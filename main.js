...
const VERSION = 'v42.1-Real-Document-Studio-Pro';
const DOCS = [
...
function updatePreview(){ if(!state?.previewCanvas)return; const p=state.previewCanvas, ctx=p.getContext('2d'); ctx.clearRect(0,0,p.width,p.height); ctx.fillStyle='#fff'; ctx.fillRect(0,0,p.width,p.height); const scale=p.width/A4.w; const top=state.topGap*scale; ctx.strokeStyle='#111'; ctx.lineWidth=1; ctx.strokeRect(0,0,p.width,p.height); ctx.setLineDash([8,5]); ctx.strokeStyle='#ff304b'; ctx.beginPath(); ctx.moveTo(0,top);ctx.lineTo(p.width,top);ctx.stroke();ctx.setLineDash([]); const crop=getCroppedCanvas(); if(state.mode==='images' && state.frontImg && state.backImg){ drawCard(ctx,crop,20*scale,top,CARD.w*scale,CARD.h*scale,'FRONT'); drawCard(ctx,crop,(20+CARD.w+8)*scale,top,CARD.w*scale,CARD.h*scale,'BACK'); } else { const x=(p.width-CARD.w*scale)/2; drawCard(ctx,crop,x,top,CARD.w*scale,CARD.h*scale,'PRINTABLE'); } }
...
function makeA4Canvas(){ const out=document.createElement('canvas'); out.width=2480; out.height=3508; const ctx=out.getContext('2d'); ctx.fillStyle='#fff';ctx.fillRect(0,0,out.width,out.height); const s=out.width/A4.w, top=state.topGap*s; const crop=getCroppedCanvas(); if(state.mode==='images' && state.frontImg && state.backImg){ drawCard(ctx,crop,20*s,top,CARD.w*s,CARD.h*s,'FRONT'); drawCard(ctx,crop,(20+CARD.w+8)*s,top,CARD.w*s,CARD.h*s,'BACK'); ctx.strokeStyle='#999'; ctx.setLineDash([12,12]); ctx.beginPath(); ctx.moveTo((20+CARD.w+4)*s, top-5*s); ctx.lineTo((20+CARD.w+4)*s, top+(CARD.h+5)*s); ctx.stroke(); } else { drawCard(ctx,crop,(A4.w-CARD.w)/2*s,top,CARD.w*s,CARD.h*s,'PRINTABLE'); } ctx.strokeStyle='#ff304b';ctx.setLineDash([12,10]);ctx.beginPath();ctx.moveTo(0,top);ctx.lineTo(out.width,top);ctx.stroke(); return out; }
