...
===================================================== */
(function(){
  const VERSION='v41.8-Document-Studio-Tools';
  ...
  function renderEditor(){
    ...
    const front=S.mode!=='back', back=S.mode!=='front';
    ws().innerHTML=header(d.name+' Printable Editor','Large workspace: upload, zoom, 8-point crop, preview, download and print.')+`
      ...
      <div class="v418-work">
        ...
        <aside class="v418-side"><div class="card"><h3>Live A4 Print Preview</h3><div class="v418-a4"><div id="v418Strip" class="v418-strip"></div></div><div class="v418-spec"><div><span>Paper</span><b>A4 Portrait</b></div><div><span>Top Gap</span><b>2.2 mm</b></div><div><span>Alignment</span><b>Top Center</b></div><div><span>Front/Back Gap</span><b>2.4 mm</b></div></div><label class="v418-copy">Copies <select id="v418Copies" onchange="v418Preview()"><option value="1">1</option><option value="2">2</option><option value="4">4</option><option value="6">6</option></select></label><div class="v418-actions bottom"><button class="blue" onclick="v418Preview()">Preview</button><button class="green" onclick="v418Download()">Download</button><button onclick="v418Print()">Print</button></div><p class="v418-warn">Print me scale <b>Actual Size / 100%</b> rakhein.</p></div></aside>
      </div>`;
  ...
  function pdfWorkspace(){return `<div id="v418PdfStage" class="v418-stage large ${S.pdfDoc?'':'empty'}">${S.pdfDoc?'<canvas id="v418PdfCanvas"></canvas>':'Upload PDF first. PDF ka selected page yahi bada preview hoga.'}</div><div class="v418-tools"><button onclick="v418Auto('pdf')">Auto Crop</button><button onclick="v418Zoom(1.2)">Zoom +</button><button onclick="v418Zoom(.84)">Zoom -</button><button onclick="v418Fit()">Fit Width</button><button onclick="v418Reset('pdf')">Reset Crop</button><button onclick="v418SetPdfSide('front')">Set Front</button><button onclick="v418SetPdfSide('back')">Set Back</button></div>`}
  ...
  async function collect(){const arr=[]; if(S.tab==='pdf'){const src=await crop(S.pdfCrop); if(src){ if(S.mode!=='back')arr.push({src,label:'FRONT'}); if(S.mode==='both')arr.push({src,label:'BACK'}); if(S.mode==='back')arr.push({src,label:'BACK'}); }} else {if(S.mode!=='back')arr.push({src:await crop(S.frontCrop),label:'FRONT'}); if(S.mode!=='front')arr.push({src:await crop(S.backCrop),label:'BACK'});} return arr.filter(x=>x.src);}
