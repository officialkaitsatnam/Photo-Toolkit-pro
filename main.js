...
/* =========================================================
   ...
   - Scan filters: Original, Auto, Magic Color, Gray, B/W
   - Brightness, Contrast, Sharpness, Rotate, Auto Crop
   ========================================================= */
(function(){
  const $ = s => document.querySelector(s);
  const sleep = ms => new Promise(r=>setTimeout(r,ms));
  const DOCS = window.DOCS || {
    aadhaar:{name:'Aadhaar Card',icon:'aadhaar.jpg'}, pan:{name:'PAN Card',icon:'pan.jpg'}, voter:{name:'Voter ID Card',icon:'voter.jpg'}, ayush:{name:'Ayushman Card',icon:'ayushman.jpg'}, abha:{name:'ABHA Card',icon:'abha.jpg'}, dl:{name:'Driving Licence',icon:'dl.jpg'}
  };
  const ST={doc:'aadhaar',mode:'both',tab:'image',front:null,back:null,pdfDoc:null,pdfPage:1,pdfPages:0,pdfScale:1.65,frontCrop:null,backCrop:null,pdfCrop:null,filter:'auto',brightness:0,contrast:8,sharp:10,top:2.2,gap:2.4,copies:1,layout:'pair',bleed:false,passport:null,passportCrop:null};
  const ratioCard=85.6/54, ratioPass=35/45;
  function toast(msg){ if(typeof window.toast==='function') window.toast(msg); else alert(msg); }
  function header(t,s){ return (typeof window.pageHeader==='function') ? window.pageHeader(t,s) : `<div class="page-title"><div><h1>${t}</h1><p>${s}</p></div></div>`; }
  function imgFile(file){return new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result);r.onerror=rej;r.readAsDataURL(file)})}
  function loadImg(src){return new Promise((res,rej)=>{const i=new Image();i.onload=()=>res(i);i.onerror=rej;i.src=src})}
  function normalize(file){return imgFile(file).then(loadImg).then(img=>{const max=2600, sc=Math.min(1,max/Math.max(img.width,img.height)); const c=document.createElement('canvas'),x=c.getContext('2d'); c.width=Math.round(img.width*sc); c.height=Math.round(img.height*sc); x.fillStyle='#fff'; x.fillRect(0,0,c.width,c.height); x.drawImage(img,0,0,c.width,c.height); return c.toDataURL('image/jpeg',.96)})}
  ...
  window.spt413Mode=m=>{ST.mode=m; render()}; window.spt413Tab=t=>{ST.tab=t; render()}; window.spt413Set=(k,v)=>{ST[k]=isNaN(Number(v))?v:Number(v); preview();};
  function render(){const d=DOCS[ST.doc]; const f=ST.mode!=='back', b=ST.mode!=='front'; workspace.innerHTML=header('Document Studio Print Engine','Exact PVC-card print + CamScanner-style cleanup tools.')+`
    <div class="card spt413-head"><div class="spt413-title"><img src="${d.icon}" onerror="this.style.display='none'"><div><h2>${d.name}</h2><p>Output: exact 85.6 × 54 mm · A4 Portrait · 300 DPI export</p></div></div><div class="spt413-actions"><button onclick="documentStudio()">← Documents</button><button class="spt413-blue" onclick="preview()">Preview</button><button class="spt413-green" onclick="downloadDoc()">Download PDF</button><button onclick="printDoc()">Print</button></div></div>
    <div class="spt413-grid"><div>
      <div class="card"><h3>2. Mode & Upload</h3><div class="spt413-tabs">${['front','back','both'].map(m=>`<button class="${ST.mode===m?'active':''}" onclick="spt413Mode('${m}')">${m==='both'?'Front + Back':m[0].toUpperCase()+m.slice(1)}</button>`).join('')}</div><div class="spt413-tabs">${['image','pdf'].map(t=>`<button class="${ST.tab===t?'active':''}" onclick="spt413Tab('${t}')">${t==='image'?'Image Upload':'Full Page PDF'}</button>`).join('')}</div>${ST.tab==='image'?uploadImages(f,b):uploadPdf()}</div>
      <div class="card"><h3>3. Crop / Scan Edit</h3>${ST.tab==='image'?cropImages(f,b):cropPdf()}${scanTools()}<div class="spt413-note">CamScanner-style filters are applied to final preview/PDF, not only the screen preview.</div></div>
    </div><aside class="spt413-side"><div class="card"><h3>4. A4 Print Preview</h3><div class="spt413-a4"><div id="spt413A4" class="spt413-strip"></div></div>${printSettings()}<div class="spt413-actions bottom"><button class="spt413-blue" onclick="preview()">Refresh Preview</button><button class="spt413-green" onclick="downloadDoc()">Download</button><button onclick="printDoc()">Print</button></div></div></aside></div>`; setTimeout(init,160); setTimeout(preview,320);}
  function uploadImages(f,b){return `<div class="spt413-upload-grid">${f?`<div class="spt413-upload"><b>Front Image</b><label><input hidden type="file" accept="image/*" onchange="spt413LoadImg(event,'front')">${ST.front?`<img src="${ST.front}">`:'Upload Front'}</label></div>`:''}${b?`<div class="spt413-upload"><b>Back Image</b><label><input hidden type="file" accept="image/*" onchange="spt413LoadImg(event,'back')">${ST.back?`<img src="${ST.back}">`:'Upload Back'}</label></div>`:''}</div>`}
  function uploadPdf(){return `<div class="spt413-upload"><b>PDF Upload</b><label><input hidden type="file" accept="application/pdf" onchange="spt413LoadPdf(event)">${ST.pdfDoc?`PDF loaded · ${ST.pdfPages} page(s)`:'Upload Full Page PDF'}</label>${ST.pdfDoc?`<div class="spt413-row"><label>Page <select onchange="spt413PdfPage(this.value)">${Array.from({length:ST.pdfPages},(_,i)=>`<option value="${i+1}" ${ST.pdfPage===i+1?'selected':''}>${i+1}</option>`).join('')}</select></label><button onclick="spt413PdfZoom(1.2)">Zoom +</button><button onclick="spt413PdfZoom(.84)">Zoom -</button><button onclick="spt413PdfFit()">Fit</button></div>`:''}</div>`}
  function cropImages(f,b){return `<div class="spt413-upload-grid">${f?`<div class="spt413-upload"><b>Front Crop</b><div class="spt413-stage ${ST.front?'':'empty'}" id="spt413FrontStage">${ST.front?`<img id="spt413FrontMedia" src="${ST.front}">`:'Upload front first'}</div><div class="spt413-row"><button onclick="spt413Auto('front')">Auto Crop</button><button onclick="spt413Rotate('front',-90)">⟲</button><button onclick="spt413Rotate('front',90)">⟳</button><button onclick="spt413Reset('front')">Reset</button></div></div>`:''}${b?`<div class="spt413-upload"><b>Back Crop</b><div class="spt413-stage ${ST.back?'':'empty'}" id="spt413BackStage">${ST.back?`<img id="spt413BackMedia" src="${ST.back}">`:'Upload back first'}</div><div class="spt413-row"><button onclick="spt413Auto('back')">Auto Crop</button><button onclick="spt413Rotate('back',-90)">⟲</button><button onclick="spt413Rotate('back',90)">⟳</button><button onclick="spt413Reset('back')">Reset</button></div></div>`:''}</div>`}
  function cropPdf(){return `<div class="spt413-stage large ${ST.pdfDoc?'':'empty'}" id="spt413PdfStage">${ST.pdfDoc?'<canvas id="spt413PdfCanvas"></canvas>':'Upload PDF first'}</div><div class="spt413-row"><button onclick="spt413Auto('pdf')">Auto Crop</button><button onclick="spt413PdfZoom(1.2)">Zoom +</button><button onclick="spt413PdfZoom(.84)">Zoom -</button><button onclick="spt413Reset('pdf')">Reset</button></div>`}
  function scanTools(){return `<div class="spt413-tools"><label>Scan Filter<select onchange="spt413Set('filter',this.value)"><option value="auto" ${ST.filter==='auto'?'selected':''}>Auto Enhance</option><option value="magic" ${ST.filter==='magic'?'selected':''}>Magic Color</option><option value="gray" ${ST.filter==='gray'?'selected':''}>Grayscale</option><option value="bw" ${ST.filter==='bw'?'selected':''}>Black & White</option><option value="original" ${ST.filter==='original'?'selected':''}>Original</option></select></label><label>Brightness <input type="range" min="-40" max="40" value="${ST.brightness}" oninput="spt413Set('brightness',this.value)"></label><label>Contrast <input type="range" min="-40" max="60" value="${ST.contrast}" oninput="spt413Set('contrast',this.value)"></label><label>Sharpness <input type="range" min="0" max="60" value="${ST.sharp}" oninput="spt413Set('sharp',this.value)"></label></div>`}
  function printSettings(){return `<div class="spt413-settings"><div><span>Paper</span><b>A4 Portrait</b></div><div><span>Card Size</span><b>85.6 × 54 mm</b></div><label>Copies <select onchange="spt413Set('copies',this.value)">${[1,2,4,6,8].map(n=>`<option value="${n}" ${ST.copies==n?'selected':''}>${n}</option>`).join('')}</select></label><label>Top mm <input type="number" step="0.1" value="${ST.top}" onchange="spt413Set('top',this.value)"></label><label>Gap mm <input type="number" step="0.1" value="${ST.gap}" onchange="spt413Set('gap',this.value)"></label><label>Layout <select onchange="spt413Set('layout',this.value)"><option value="pair" ${ST.layout==='pair'?'selected':''}>Front-Back Side by Side</option><option value="stack" ${ST.layout==='stack'?'selected':''}>Stacked Rows</option></select></label></div><div class="spt413-note">Printer setting: A4 + Actual size / 100%. Do not select Fit to page.</div>`}
  window.spt413LoadImg=async(e,side)=>{const f=e.target.files[0]; if(!f)return; ST[side]=await normalize(f); ST[side+'Crop']=null; render(); setTimeout(()=>spt413Auto(side),400)};
  window.spt413LoadPdf=async e=>{const f=e.target.files[0]; if(!f)return; if(!window.pdfjsLib)return toast('PDF library not loaded. Refresh once.'); const ab=await f.arrayBuffer(); ST.pdfDoc=await pdfjsLib.getDocument({data:ab}).promise; ST.pdfPages=ST.pdfDoc.numPages; ST.pdfPage=1; ST.pdfCrop=null; render(); setTimeout(renderPdf,350)};
  window.spt413PdfPage=v=>{ST.pdfPage=Number(v);ST.pdfCrop=null;render();setTimeout(renderPdf,250)}; window.spt413PdfZoom=f=>{ST.pdfScale=Math.max(.55,Math.min(4,ST.pdfScale*f));renderPdf()}; window.spt413PdfFit=()=>{ST.pdfScale=1.35;renderPdf()};
  async function renderPdf(){const c=$('#spt413PdfCanvas'); if(!c||!ST.pdfDoc)return; const p=await ST.pdfDoc.getPage(ST.pdfPage); const vp=p.getViewport({scale:ST.pdfScale}); c.width=vp.width;c.height=vp.height; await p.render({canvasContext:c.getContext('2d'),viewport:vp}).promise; await sleep(40); if(!ST.pdfCrop) ST.pdfCrop=createCrop($('#spt413PdfStage'),c,ratioCard); else paint(ST.pdfCrop); preview();}
  function init(){if(ST.front&&$('#spt413FrontMedia')&&!ST.frontCrop)ST.frontCrop=createCrop($('#spt413FrontStage'),$('#spt413FrontMedia'),ratioCard); if(ST.back&&$('#spt413BackMedia')&&!ST.backCrop)ST.backCrop=createCrop($('#spt413BackStage'),$('#spt413BackMedia'),ratioCard); if(ST.pdfDoc&&$('#spt413PdfCanvas'))renderPdf();}
  ...
  function clamp(c,x=c.x,y=c.y,w=c.w,h=c.h){const b=visible(c.stage,c.media),r=c.ratio; if(w/h>r)h=w/r; else w=h*r; w=Math.max(70,Math.min(w,b.w)); h=Math.max(45,Math.min(h,b.h)); if(w/h>r)h=w/r; else w=h*r; if(w>b.w){w=b.w;h=w/r} if(h>b.h){h=b.h;w=h*r} x=Math.max(b.x,Math.min(x,b.x+b.w-w)); y=Math.max(b.y,Math.min(y,b.y+b.h-h)); return{x,y,w,h}}
  function createCrop(stage,media,ratio){if(!stage||!media)return null; stage.querySelectorAll('.spt413-crop').forEach(e=>e.remove()); const b=visible(stage,media); let w=b.w*.88,h=w/ratio; if(h>b.h*.86){h=b.h*.86;w=h*ratio} const c={stage,media,ratio,x:b.x+(b.w-w)/2,y:b.y+(b.h-h)/2,w,h}; const box=document.createElement('div'); box.className='spt413-crop'; ['nw','n','ne','e','se','s','sw','w'].forEach(n=>{const h=document.createElement('span');h.className='spt413-h '+n;h.dataset.h=n;box.appendChild(h)}); const m=document.createElement('i');m.textContent='MOVE';box.appendChild(m); c.box=box; stage.appendChild(box); attach(c); paint(c); return c;}
  function paint(c){if(!c||!c.box)return; Object.assign(c,clamp(c)); Object.assign(c.box.style,{left:c.x+'px',top:c.y+'px',width:c.w+'px',height:c.h+'px'})}
  function attach(c){let mode,start={}; const pt=e=>e.touches?{x:e.touches[0].clientX,y:e.touches[0].clientY}:{x:e.clientX,y:e.clientY}; function down(e){e.preventDefault(); const p=pt(e); mode=e.target.dataset.h||'move'; start={px:p.x,py:p.y,x:c.x,y:c.y,w:c.w,h:c.h}; document.addEventListener('mousemove',move);document.addEventListener('mouseup',up);document.addEventListener('touchmove',move,{passive:false});document.addEventListener('touchend',up)} function move(e){e.preventDefault(); const p=pt(e),dx=p.x-start.px,dy=p.y-start.py; let x=start.x,y=start.y,w=start.w,h=start.h; if(mode==='move'){x+=dx;y+=dy}else{if(mode.includes('e'))w+=dx;if(mode.includes('s'))h+=dy;if(mode.includes('w')){x+=dx;w-=dx}if(mode.includes('n')){y+=dy;h-=dy}} Object.assign(c,clamp(c,x,y,w,h)); paint(c)} function up(){document.removeEventListener('mousemove',move);document.removeEventListener('mouseup',up);document.removeEventListener('touchmove',move);document.removeEventListener('touchend',up);preview()} c.box.addEventListener('mousedown',down); c.box.addEventListener('touchstart',down,{passive:false});}
  window.spt413Auto=kind=>{let c=ST[kind+'Crop']; if(!c){init();c=ST[kind+'Crop']} if(!c)return; const b=visible(c.stage,c.media); let w=b.w*.92,h=w/c.ratio; if(h>b.h*.88){h=b.h*.88;w=h*c.ratio} Object.assign(c,{x:b.x+(b.w-w)/2,y:b.y+(b.h-h)/2,w,h}); paint(c); preview(); toast('Auto crop applied')};
  window.spt413Reset=kind=>{ST[kind+'Crop']=null;render()};
  window.spt413Rotate=async(side,deg)=>{if(!ST[side])return; const img=await loadImg(ST[side]); const c=document.createElement('canvas'),x=c.getContext('2d'); c.width=Math.abs(deg)%180===90?img.height:img.width; c.height=Math.abs(deg)%180===90?img.width:img.height; x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);x.translate(c.width/2,c.height/2);x.rotate(deg*Math.PI/180);x.drawImage(img,-img.width/2,-img.height/2); ST[side]=c.toDataURL('image/jpeg',.96); ST[side+'Crop']=null; render();};
  async function cropCard(crop){if(!crop)return null; const media=crop.media; let source,nw,nh; if(media.tagName==='CANVAS'){source=media;nw=media.width;nh=media.height}else{source=await loadImg(media.src);nw=source.width;nh=source.height} const mr=media.getBoundingClientRect(),sr=crop.stage.getBoundingClientRect(); let rx=(crop.x-(mr.left-sr.left))/mr.width,ry=(crop.y-(mr.top-sr.top))/mr.height,rw=crop.w/mr.width,rh=crop.h/mr.height; rx=Math.max(0,Math.min(1,rx));ry=Math.max(0,Math.min(1,ry));rw=Math.max(.01,Math.min(rw,1-rx));rh=Math.max(.01,Math.min(rh,1-ry)); const out=document.createElement('canvas'),x=out.getContext('2d'); out.width=1600;out.height=Math.round(out.width/ratioCard); x.fillStyle='#fff';x.fillRect(0,0,out.width,out.height); x.drawImage(source,nw*rx,nh*ry,nw*rw,nh*rh,0,0,out.width,out.height); applyScan(x,out.width,out.height); return out.toDataURL('image/jpeg',.96)}
  function applyScan(ctx,w,h){let img=ctx.getImageData(0,0,w,h),d=img.data; const mode=ST.filter; const br=Number(ST.brightness), ct=Number(ST.contrast)+(mode==='auto'?10:mode==='magic'?18:0), factor=(259*(ct+255))/(255*(259-ct)); for(let i=0;i<d.length;i+=4){let r=d[i],g=d[i+1],b=d[i+2]; if(mode==='gray'||mode==='bw'){let y=.299*r+.587*g+.114*b; r=g=b=y} if(mode==='magic'||mode==='auto'){r=Math.min(255,r*1.06);g=Math.min(255,g*1.07);b=Math.min(255,b*1.03)} r=factor*(r-128)+128+br; g=factor*(g-128)+128+br; b=factor*(b-128)+128+br; if(mode==='bw'){const y=(r+g+b)/3>148?255:0;r=g=b=y} d[i]=Math.max(0,Math.min(255,r));d[i+1]=Math.max(0,Math.min(255,g));d[i+2]=Math.max(0,Math.min(255,b)); } ctx.putImageData(img,0,0); if(Number(ST.sharp)>0){ctx.globalAlpha=Math.min(.45,Number(ST.sharp)/100);ctx.drawImage(ctx.canvas,-1,0);ctx.drawImage(ctx.canvas,1,0);ctx.drawImage(ctx.canvas,0,-1);ctx.drawImage(ctx.canvas,0,1);ctx.globalAlpha=1;}}
